...
- Podman - аналог Docker, позволяет запускать контейнеры без эскалации прав (rootless). Совместим с Docker на уровне команд CLI
- Docker Compose - позволяет запускать несколько разных контейнеров на одной машине, используя один управляющий скрипт
- Kubernetes - позволяет развертывать множество контейнеров на разных машинах, используя комплексные стратегии развертывания. Следует заметить, что в Kuber используется containerd - небольшой, сфокусированный runtime‑слой, который непосредственно управляет контейнерными процессами, образами и сетью. Начиная с Kubernetes 1.20 поддержка Docker поэтапно исключалась

>Существует и другие проекты, влияющие на экосистему контейнеров. The Cloud Native Copmputing Foundation (CNCF) является проектом, который был запущен как Linux Foundation project в 2015 году. На этом стандарте базируются: Kubernetes, containerd, Notary, Prometheus, Cilium и другие.
>
>Компания Docker также создала The Moby Project как платформу для community-разработчиков.

Рекомендуется для ознакомления книга Найджела Пультона (Nigel Poulton) "Docker Deep Dive" (2016). В последующие годы появилось ещё, как минимум, две редакции книги.

## Стартовые команды

Для проверки наличия установленной версии Docker используется команда: `docker version`

Список загруженных образов Docker можно получить командой: `docker images`

Загрузить из репозитария образ можно командой: `docker pull nginx:latest`

Типовая команда запуска контейнера:
