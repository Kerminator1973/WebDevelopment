...
Колоссальное влияние ИИ на разработку программного коду в 2025-2026 годах уже не подвергается сомнению. На данный момент, не актуален вопрос: применять ли ИИ для разработки ПО? Игнорирование ИИ приводит к потере конкуретоспособности коллектива.

Однако крайне важными остаётся объективная оценка ожиданий от применении ИИ и выбор модели использования ИИ.

## Опасные ожидания

В маркетинговых материалах компаний специализирующихся на внедрении ИИ приводятся оценки роста производительности работы программиста в 2x, 5x, 10x раз. Однако маркетинговые материалы часто игнорируют совокупность процессов, приводящих к разработке надёжного, быстрого, легко модифицируемого решения, фокусируясь только непосредственно на генерации кода. Следствием этого являются критически завышенные ожидания, которые не трансформируются в реальные преимущества для бизнеса.

Работа по созданию программного продукта требует:
...
- кодирования (создание кода)
- оценки эффективности полученного решения в сравнении с альтернативами (benchmarking)
- тестирования программного обеспечения, включая Unit-тестирование и автоматизированное тестирование
- устранение найденных ошибок, включая ошибки выборе архитектуры и инструментальных средств
- переработка решения по замечаниям заказчика, в том числе - пересмотр требований после ознакомления с полученным продуктом
- оптимизация решения под фактически используемый IT-ландшафт
- документирование полученного результата, включая пользовательскую документацию
- включение разработанной системы в pipeline автоматизированной сборки и доставки продукта потребителю (CI/CD)

В общем случае, кодирование занимает всего 1-5% рабочего времени программиста. Повышая эффективность только этого типа задач, мы получаем повышение производительности на уровне шума, с риском критичной деградации программного кода.

В КБ ДОРС ИИ применяется для решения следующих задач:

- быстрое прототипирование с целью проверить гипотезы (архитектура, инструментальные средства, эффективность алгоритмов)
- обучение исполнителей выбранным инструментальным средствам
- разработка рутинного кода с низкой сложностью
- подготовка приложения к включению к pipeline автоматизированной сборки
...
В 2025 году распространённых моделей использования LLM (больших языковых моделей) для разработки кода всего три:

- Chat с LLM через терминал (CLI), или через приложение в браузере/мобильном телефоне
- Plug-in в среде разработки ПО. Например, Copilot в Visual Studio, или GigaChat в GigaIDE (который базируется на JetBrain IntelliJ IDEA)
- с использованием агентов, подключаемых по протоколу MCP

Термин "vibe-coding" относится ко второму пункту, ИИ компонент встроенный в IDE.

В нашей посведневной практике, в КБ ДОРС мы не используем агентов, т.к. пока не разработаны необходимые регламенты и не создана необходимая инфраструктура. 

Пример типовой ошибка при использовании MCP-агентов: тестировщик Наташа сообщает программисту Роману, что необходимо добавить в таблицу "Devices" базы данных дополнительное поле "IsDeleted". Роман пишет корректный промпт для добавления поля в базу данных. LLM формирует корректный SQL-запрос и передаёт его MCP-агенту, который подключается к СУБД и успешно добавляет новое поле. Но всё это происходит в тестовом контуре, IT-ландшафт которого абсолютно отличается от промышленной эксплуатации. Поскольку LLM ничего не знает об IT-ландшафте в котором работает промышленная система, не знает регламентов КБ ДОРС и ДИТ, действия, необходимые для корректной публикации приложения в пром не сделаны, попытка публикации в 100% случаев ломает промышленно эксплуатируемую систему.
...
- требуются дорогостоящие мощные LLM

Рекомендации для команд разработчиков ПО по внедрению ИИ:

- Этап №1: Использовать платные агрегаторы для доступа к различным современным LLM. В КБ ДОРС используются [ChadGPT.ru](https://chadgpt.ru/) и [Merlin](https://www.getmerlin.in/ru)
- Этап №2: внедрение vide-coding для создания прототипов
- Этап №3: использование MCP-агентов для выполнения комплексных задач

>ChadGPT является типовым российским агрегатором LLM с подписочной моделью. Предлагает доступ к современным LLM: Gemini 3 Pro, Grok 4.1, GPT-5.2, Gloude 4.5 Opus, DeepSeek V3.2. Подписка на тарифном плане "Плюс" составляет 1690 руб/месяц (~900 тысяч слов на простых моделях).
