...
- подготовка приложения к автоматизированной сборке и развертыванию
...
## Выбор способа применения ИИ

Мы рассматриваем подходы к использованию больших языковых моделей (LLM) для разработки кода:

- Chat с LLM через браузер/приложение в мобильном телефоне
- Chat в специализированном Desktop-приложении. Отличие от браузера состоит в том, что Desktop-приложение имеет доступ к файловой системе и в нем удобно предоставлять доступ к локальным ресурсам, значительно упрощая создание контекста запроса. Примеры: Gemini CLI
...
Термин "vibe-coding" относится к агентскому режиму и предполагает итеративный сценарий работы:

- программист описывает желаемый результат, уточняет способ его достижения, формирует контекст
- LLM генерирует программный код и/или управяющие команды для терминала/shell-а
- программист проверяет полученный результат и вносит корректирующие указания при переходе на следующую итерацию
...
- требует дорогостоящих, мощных LLM, т.к. слабые модели очень быстро начинают галлюционировать (генерировать код содержащий критичные ошибки)

### Выбор языковой модели (LLM)

Характерным для КБ является использование различных языков и парадигм программирования, множества различных библиотек и framework-ов. Причина этого - большинство задач выполняемых КБ носят интеграционный характер, что предполагает встраивание решения в IT-ландшафт заказчика ПО. На практике, разработка ПО в КБ ведётся на языках программирования: Си, C++, C#, JavaScript, TypeScript, Python, Lua, Kotlin, Java. Типы решаемых задач - от разработки прошивок для микроконтроллеров до высоконагруженных серверных решений. Такое технологическое разнообразие приводит к тому, что становится сложно выбрать только одну LLM, которая прекрасно справляется со всеми используемыми технологиями. По этой причине, при разработке разных продуктов часто используются разные LLM.
...
>ChadGPT.ru является типовым российским агрегатором LLM с подписочной моделью. Предлагает доступ к современным LLM: Gemini 3 Pro, Grok 4.1, GPT-5.2, Glaude 4.5 Opus, DeepSeek V3.2. Подписка на тарифном плане "Плюс" составляет 1690 руб/месяц (~900 тысяч слов на простых моделях).
>
>[Молекула AI](https://moleculai.ru/model/qwen) - то же самое, что и остальные, но предоставляет доступ к китайским LLM: Kimi, Qwen и DeepSeek. Стоимость - около 1700 рублей/месяц.
>
>[ZVENOAI](https://zveno.ai/) - тоже самое, с китайскими Kimi K2.5, GLM 5, MiniMax M2.5, Nemotron 3 Super, Qwen 3, и т.д. Похоже на Ollama в облаке.
>
>[СПЕШУ AI](https://speshu.ai/) - тоже самое, с самыми современными американскими и китайскими нейронками, включая Groq Build и Qwen 3.7 MAX. Похоже на ChadGPT
...
Доступ к LLM возможен через "белый VPN", например, Kaspersky Secure Connection. Можно получить доступ к ChatGPT, но для доступа к продуктам Anthropic необходима авторизация на мобильном телефоне через SMS-код. Авторизация для граждан РФ не поддерживается. Без VPN возможен доступ к китайским LLM, но некоторые из них также требуют авторизацию. В частности, Kimi требует авторизацию либо через SMS (не доступно для граждан РФ), либо через Google-аккаунт.

Бесплатные тарифные планы LLM имеют критичные ограничения как по мощности доступных LLM, так и по количеству доступных запросов.
...
Особенность обучения нейронных сетей состоит в том, что закрепление материала, превращение его в шаблон (pattern) происходит при возникновении множества повторов. Современные LLM умеют очень хорошо генерировать простые web-приложения на Node.js, или примеры кода машинного обучения на Python. Это происходит потому, что именно таких примеров в обучающих наборах LLM больше всего.

Если у LLM оказывается недостаточно примеров, она начинает генерировать код, фактически, из случайных, слабо связанных кусков кода и это приводит к галлюцинациям.

Стоит заметить, что из условных 10000 "одинаковых" примеров кода, может сформироваться несколько самостоятельных кластеров, поскольку существуют приципиальные отличия:
...
Соответственно, LLM при генерации кода может выбирать кластер, практически, случайно. Для управления выбором кластера необходимо либо точно закладывать соотвествующие особенности в запрос, либо добаваться нужного вида кода с помощью последующих уточнений.

## Типовые задачи, которые выполняют программисты при разработке ПО 
