...
## История с покупкой Bun компанией Anthropic
...
К концу июля 2026 года туман частично рассеился и ситуация выглядит следующим образом:

- Bun является критически важной частью IT-инфраструктуры Anthropic и именно потребность контроля над ключевым инструментарием являлось причиной покупки
- Язык Zig содержал некоторые фундаментальные ограничения, которые не позволяли Anthropic достичь желаемых критериев качества
- Anthropic оказывал давление на разработчиков Zig для изменения свойств языка, но разработчики Zig отказались вносить изменения
- Anthropic принял спекулятивное решение - портировать Bun с Zig на Rust, используя Claude Code
- портирование заняло 11 дней и обошлось в $160000. Однако все тесты удалось пройти
- код сильно не эффективный, в нём есть куча Unsafe-мест, но это отправная точка

Если исходить из утверждения, что вариантов повлиять на разработчиков Zig не было, то портирование на Rust это очень рисковый, драматичный, но очень смелый ход.

Информация взята из материалов блогера Миши Ларченко на YouTube (https://larchanka.com/).

Таким образом, скорее всего Bun в моменте сильно просядет по качеству, но постепенно ситуация будет улучшаться и, в конце концов, станет лучше, чем было с Zig-версией.
