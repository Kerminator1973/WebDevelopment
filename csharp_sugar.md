...
В старых библиотеках, например, Json.NET реализованы для работы с UTF-16. Однако, более высокая производительность могла бы быть дрстигнута с использованием кодировки UTF-8 (используется по умолчанию в приложениях .NET), поскольку это позволило бы избежать дополнительное перекодировки JSON-файлов из Json.NET Unicode-16 в UTF-8.
