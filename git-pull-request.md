...
## Основной репо изменяется, как синхронизовать его с моим fork-ом

Основной репозиторий как удалённый называется **upstream**.

Сначала нужно добавить основной репозиторий как удалённый источник (remote). Это можно сделать командой:

```shell
git remote add upstream https://github.com/ORIGINAL_OWNER/ORIGINAL_REPO.git
```

Посмотреть все remote-источники можно командой:
...
Это скачает изменения, но не применит их к локальным веткам. Опционально нужно переключиться на нужную ветку (например на main):

```shell
git checkout main
# или master — зависит от проекта
```

Затем следует влить изменения из upstream в свою ветку. Это можно сделать двумя разными способами.

- Merge — сохраняет историю, добавляет merge-коммит: `git merge upstream/main`
- Rebase — делает историю линейной (рекомендуется для форков): `git rebase upstream/main`

Затем нужно отправить изменения на свой fork на GitHub:

```shell
git push origin main
# если был сделан rebase — может потребоваться:
