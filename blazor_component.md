...
## Каскадные параметры (Cascading Parameters)
...
Этот подход рассматривается, как дополнительный механизм передачи данных через параметры компонента.
...
Стоит повторить: **CascadingValue** - это компонент-обёртка, определяемый в родительском компоненте. **CascadingParameter** - атрибут, который помечает свойство в дочернем компоненте для приёма каскадного значения.

Ключевые преимущества:

- Упрощение кода. Не нужно прокидывать одни и те же параметры через цепочку компонентов
- Удобство поддержки. Изменение значения происходит в одном месте — в источнике каскада
- Гибкость. Данные доступны всем потомкам, независимо от глубины вложенности
- Снижение связанности. Компоненты не зависят напрямую друг от друга, а используют общий контекст
...
При изменении значения в <CascadingValue> Blazor автоматически вызывает `StateHasChanged`() у всех потомков с соответствующими [CascadingParameter], что приводит к перерисовке.

При передаче больших объектов или часто изменяемых данных каскадные параметров могут вызвать избыточные перерисовки.

### Каскадирование - практическое применение

Механизм удобно использовать для поддержки тем пользовательского интерфейса, поскольку каждый конкретный компонент должен реализовать рендеринг себя с учётом текущей установленный темы:
...
Другие случаи, в который каскадные значения могут быть полезны:

- Распространение информации о текущем пользователе (логин, права доступа) по иерархии
- Предоставление сервисов или зависимостей (например, экземпляра HttpClient) без DI‑инъекции в каждый компонент
