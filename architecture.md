...
Предположим, что у нас есть система с 1000 функций. В одной из функций была найдена ошибка. В случае монолита потребуется обновлять всю системы при исправлении каждой ошибки. Если система разделена, например, на 200 сервисов, то при исправлении ошибки в одном сервисе, необходимо переопубликовать только этот сервис.
...
Основная проблема развертывания связана с разным объёмом вычислений необходимых для разных функций системы. Например, операция авторизации выполняется один раз на пользовательскую сессию и может быть достаточно одного виртуального сервера для выполнения этой задачи. А задач генерации пользовательского интерфейса системы может выполняться, например, по 500 штук в секунду и для обеспечения приемлемого времени отклика необходимо 10 виртуальных серверов. Если у нас монолит, то нам потребуется развернуть 11 монолитов на каждую из виртуальных машин, что может быть избыточным, из-за повышенных требований к ресурсам монолита.
...
Адвокаты SOA пропагандируют следующие манифест приоритетов:
