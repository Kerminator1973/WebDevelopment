...
>Для проверки эффективности реализации API (время отклика, латентность, и т.д.) используются такие решения как **NGINX's Controller** и [Tyk](https://tyk.io/).
...
## Как можно выявлять Faults в системе

Существует несколько распространённых подходов:

- Sidecar design pattern / Envoy (агент, довренное лицо). Дополнительный слой добавляется к приложению и выполняет дополнительные действия, в частности - выполняет мониторинг кода ответа сервисов (5XX). Envoy может самостоятельно повторять попытки доставки сообщений серверу, или даже реализовывать действия по обеспечению катастрофоустойчивости
- Heartbeat mechanism. Регулярный обмен сообщениями "я жив" между двумя сервисами
- Leaky bucket counter. В приложение добавляется компонент, который работает как дырявое ведро для воды: если возникает ошибка, счётчик увеличивается на единицу. Каждый период времени счётчик уменьшается на единицу. Если "ведро" заполняется, система считает это критичным сбоем и реагирует на него. Алгоритм хорошо ловит ситуацию, в которой возникает большое количество проблем за короткий период времени
