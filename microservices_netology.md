...
Автор лекции ссылается на продукты для сборки и анализа логов из экосистемы **Elastic**.

Часто ставят локально демона: [logstash](https://www.elastic.co/logstash), [vector](https://vector.dev/), который собирает stdout и пересылает на сервер. logstash - проект компании Elastic.

[ElasticSearch](https://www.elastic.co/elasticsearch): часто используется для анализа логов. ElasticSearch имеет как бесплатную, так и платные версии. Базовая функциональность доступна бесплатно под лицензией Apache 2.0, но расширенные возможности требуют платной подписки. 

[Kibana](https://www.elastic.co/kibana): консоль для формирования запросов к системе логирования.
...
Повтор запроса - весьма эффективная стратегия, т.к. есть высокая вероятность того, что повтор попадёт на другой instance и корректно обработается. Однако важно избежать ситуации, когда не аккуратно реализованные повторы вызывают избыточную нагрузку на систему. Чтобы избежать этого, используется подход с увеличивающимся периодом повтора. Например, первый повтор через 30 секунд, второй через одни минуту, а третий - через 2 минуты. Иногда время повтора вычисляется по квадратичная, или экспотенциальной формуле, чтобы предотвратить деградацию системы.

Circuit Breaker (англ. "прерыватель цепи", "автоматический выключатель") — это шаблон проектирования, который предотвращает каскадные сбои в распределённых системах, в том числе в микросервисных архитектурах. Он работает аналогично электрическому выключателю: при обнаружении проблем разрывает "цепь" вызовов к проблемному сервису, чтобы не перегружать систему и не ухудшать пользовательский опыт.

Bulkhead (англ. "переборка", "перегородка") — это шаблон проектирования, который изолирует компоненты системы друг от друга, чтобы сбой в одном из них не привёл к отказу всей системы.

Название происходит от водонепроницаемых переборок в корпусе корабля: при повреждении одного отсека он заполняется водой, но остальные отсеки остаются герметичными — и судно не тонет.

## Балансировка

Что ещё может делать балансировщик, кроме непосредственно балансировки нагрузки:
...
- Кэширование HTTP
- Контекстно-зависимое распределение запросов
- Аутентификация клиентов
- Фильтрация контента

В общем случае, внешний трафик - HTTPS, а внутренний - HTTP.

Распространённые балансировщики: Nginx, F5 ("железный"), Kemp, HAProxy, Envoy
...
Альтернативный способ - системы обработки задач с использованием брокера сообщений. В системе есть генераторы и обработчики сообщений.

## Кэширование

Кэширование - наиболее распространённый способ повышения производительности.

Виды cache:

- клиентский cache - клиент хранит cache и использует его, чтобы не выполнять дополнительных запросов на сервер
- сквозной cache - cache хранится на _reverse proxy_
- прогретый cache - сервер заранее готовит данные для клиента и сохраняет их в своём cache (например, справочники)

Распределённый cache - использование in-memory база данных на специальных машинах. Примеры in-memoty СУБД: Redis, Memcached, DragonflyDB (полностью совместимо с Redis), Valkey (форк Redis, развивается под управлением Linux Foundation), Apache Ignite, Tarantool DB (российская in-memory СУБД), Hazelcast (распределённая), Aerospike (высокопроизводительная, распределённая).

## Автомасштабирование

Автоматическое масштабирование - процесс динамического выделения ресурсов для удовлетворения требований производительности: при увеличении нагрузки выделяются дополнительные ресурсы для обеспечения соглашения об уровне обслуживания (SLA).
...
Подходы для организации автомасштабирования:

- Динамическое масштабирование на основе метрик приложения, или инфраструктуры: среднее значение загрузки CPU, общее количество запросов, длительность обработки задач, количество запросов, завершившихся ошибкой
- Масштабирование по расписанию
- Предсказательное масштабирование. Часто используются нейронные сети и ML для анализа исторических данных

Решения облачных провайдеров:
...
Service Discovery может осуществляться разными способами:

- У клиентов в конфигурации IP-адреса сервисов, с которыми ему нужно взаимодействовать (например, в "appsettings.json")
- У каждого сервиса может быть DNS-имя, с которым связано несколько IP-адресов его запущенных экземпляров
- Service Registry - специализированного программное обеспечение, которое позволяет сервисам на стартре зарегистрировать адреса запускаемого экземпляра, а клиентам получить эти адреса по имени
...
Набирает популярность подход Service Mash:
