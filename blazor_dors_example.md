...
> StateHasChanged уведомляет компонент о том, что его состояние изменилось. При необходимости вызов StateHasChanged вызывает rerender, который возникает, когда основной поток приложения свободен. StateHasChanged вызывается автоматически для методов EventCallback.
