...
- может быть побочка от наследования компонентов - Blazor может потерять связь между базовым и производным классов и не увидит изменения
- после выполнения Action-а из дочернего компонента

Добавление StateHasChanged() в код приложения чаще всего приходится осуществлять ситуативно, т.е. в тех случаях, когда по факту изменение состояния произошло, а re-rendering не случился. Данная особенность связана с тем, что Blazor поддерживает многопоточное выполнение - его Runtime существенно более сложный, чем Runtime JavaScript.
