...
Костыль/обманка с обязательным добавлением в список обязательного Default-ного значения работает, но почему то, после первого открытия элемента списка (см. isOpen) второй раз этот подход не срабатывает. Однако если перейти на закладку "Weather", или "Home", а потом вернуться, то список снова начнёт автоматически раскрываться.

> Update "воскресения": очень похоже на то, что не может быть двух раскрытых списков одновременно. Если при выборе элемента списка закрывать этот список (в обработчике события OnSelectionChanged), то FluentUI будет работать корректно. Однако если выбирать уже выбранный элемент, то обработчик OnSelectionChanged повторно приходить не будет.
