...
> При отладке кода столкнулся со странной ситуацией: точка останова не страбатывает в Visual Studio 2022, но сам код выполняется и можно увидеть вывод в консоль браузера, который осуществляется строкой: `Console.WriteLine($"Selected model: {model}");`. Возможно, нужно запустить специальный режим отладки, нажав: Shift+Alt+D
...
Вот и всё - когда мы выбираем значение в одном списке, автоматически раскрывается следующий!
...
Однако мы получаем несколько проблем:
...
В Blazor, компонент FluentSelect спроектирован таким образом, чтобы событие OnSelectionChanged возникало только в том случае, если был выбран элемент
отличный от выбранного ранее. Такое поведение может быть не удобным, если важен сам факт выбора, даже если элемент не меняется. Например: при щелчке на модели прибора, нужно автоматически раскрывать следующий список со списком стран.

Обойти ограничение можно обрабатывая onclick элемента списка. По каким-то причинам, событие OnSelectionChanged блокирует событие onclick дочернего элемента, но если элемент не поменялся, то FluentSelect вызывает обработчик oncliсk. Этот подход рабочий, но его сложно назвать рабочим, поскольку он построен на недокументированных особенностях поведения FluentUI.

К сожалению, на практике этот подход использовать нельзя из-за побочных эффектов:

- исчезает возможность выполнять выбор элемента списка с клавиатуры
- перестаёт надёжно работать возможность установки начального значения в списке (через SelectItem)

### Свойство Open работает, но только у одного списка оно должно быть установлено в true

Особенность FluentUI состоит в том, что только у одного списка свойство **Open** может быть установлено в значение true. При попытке установить лишнее свойство, функционал автоматического открытия списка при возникновении некоторого события может перестать работать. На практике, если в обработчике события OnSelectionChanged нужно открыть другой список, сначала нужно "закрыть" текущий список, установив связанное свойство (isOpen) в значение false.
...
Костыль/обманка с обязательным добавлением в список обязательного Default-ного значения работает.

> StateHasChanged уведомляет компонент о том, что его состояние изменилось. При необходимости StateHasChanged() вызывает re-render, который возникает, когда основной поток приложения свободен. StateHasChanged вызывается автоматически для методов EventCallback.

Существуют ситуации, в которых StateHasChange() необходимо вызывать вручную, т.к. Blazor не может сам распознавать автоматически. Чаще всего такая ситуация происходит в следующих случаях:

- изменение происходит в асинхронных функциях, после await, не в UI-контексте
- приходят события не являющиеся Blazor-овскими, например, события в JavaScript о срабатывании таймера
- может быть побочка от наследования компонентов - Blazor может потерять связь между базовым и производным классами и не увидит изменения
- после выполнения Action-а из дочернего компонента

Добавление StateHasChanged() в код приложения чаще всего приходится осуществлять ситуативно, т.е. в тех случаях, когда по факту изменение состояния произошло, а re-rendering не случился. Заметим, что Blazor не поддерживает полноценное многопоточное выполнение, что связано с ограничениями WebAssembly, т.е. Blazor на клиенте работает в однопоточном режиме. Проблемы с выявление изменения состояния связаны с тем, что Blazor делает это автоматически, тогда как, например, в React необходимо вызывать функцию, полученную посредством useState() для явного изменения состояния.

Для многопоточного выполнения кода можно попробовать использовать Web.Worker.
...
Также необходимо выполнить оптимизацию вызовов **StateHasChanged**(). Оптимальным кажется вызов метода компонента в конце обработки события. Например, это можно сделать следующим образом:
