...
- .NET Core выполняет приложения с высокой скоростью (TODO: _объективное сравнение и разобраться с тем, что такое dotnet.native.wasm_)
...
- используемый подход может помешать использованию таких мощных и надёжных библиотек, как [DataTables.NET](https://datatables.net/)
...
TODO: _нужно разобраться как на самом деле, но пока картина выглядит так - загружается и работает **реализация .NET на wasm**_ Т.е. байт-код .NET транслируется в wasm, а затем транслируется в Native-код. Если в Developer Console F12 на закладке "Console" раскрыть пункт "**dotnet** Loaded xx.xx Mb resources", то появится список загруженных компонентов. При первой загрузке это будут dll-файлы, в потом, извлечённые из cache это будут уже скомпилированный wasm-файлы. Очень легко заметить, что имена файлов (без расширений) будут на 100% одинаковыми.
...
Есть у запущенного в браузере Blazor-приложения открыть Developer Console F12, то в разделе "Console" можно увидеть следующее сообщение: `Debugging hotkey: Shift+Alt+D (when application has focus)`

Мне не удалось запустить отладку клиентского кода из под Visual Studio, но когда я запустил Visual Studio Code, IDE предложил мне поставить Debugging Assets. После этого я запустить приложение командой `dotnet run` и открыл приложение по ссылке.
