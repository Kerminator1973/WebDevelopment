...
Проблемой традиционных web-приложений является слишком грубое разделение на три части: структура документа (HTML), представление (CSS) и динамический код. В реальных приложениях такой степени изоляции может быть недостаточно. Предположим, что web-приложение управляет несколькими органами управления, что часто означает, что для всех органов управления используется один HTML-документ, одна каскадная таблица и один JS-файл. Для того, чтобы разрабатывать и сопровождать web-приложения, было бы крайне желательно разбить разметку (HTML), стили (CSS) и динамическое поведение (JS) на изолированные компоненты и работать над каждым из них по отдельности.

Таким образом внедрение компонентной модели является решением проблемы управляемости разработкой web-приложения. Компонентная модель используется, в частности, в React, Angular, Vue.js и Svelte.
...
Следует заметить, что языком разметки для обоих режимов работы является **Razor** (как и для ASP.NET Core).
...
Blazor поддерживает Ahead-of-Time (AOT) компиляцию, т.е. транслирует сборки .NET в бинарный код WebAssembly, что означает, что клиентская машина (браузер) исполняет WebAssembly-код. Если AOT не разрешён, что Blazor запускает приложение в режиме интерпретации, что приводит к значительному замедлению кода. Главный недостаток AOT состоит в том, что скомпилированные приложения весят больше, чем интерпретируемая IL-сборка.
