...
Ещё один крайне важный аспект разработки пользовательского интерфейса - выбор подхода программирования: декларативный, или императивный (директивный). Пример императивного подхода - jQuery с JavaScript: при возникновении некоторого события, работает код, который явным образом формирует DOM. Декларативный подход состоит в том, что инструмент описывает разметку и привязки к данным. В рамках декларативного подхода, крайне сложно написать код, который добавит один и тот же блок в верстку дважды, тогда как в императивном подходе такое случается сплошь и рядом. Декларативный подход более защищён от ошибок в логике.
...
- в режиме Blazor Server не нужно разрабатывать REST API и не нужно реализовывать state management для того, чтобы выполнять запросы на сервер
- при использовании Blazor Server, достигается высокая утилизация вычислительных ресурсов, т.к. задачи легко распараллеливаются
- .NET Core выполняет приложения с высокой скоростью (в режиме wasm, при незначительном JS Interop)
...
- **Streaming Rendering** позволяет очень быстро загрузить статические элементы страницы, отобразить их, а затем начать загружать динамические элементы, получение которых может занимать значительное количество времени
- можно совмещать разные модели (Blazor WebAssembly и Blazor Server) в одном приложении, см. `@attribute [RenderModeServer]`, `@attribute [StreamRendering(true)]`, `@attribute [RenderModeAuto]`
- Razor Components Result - некоторые компоненты могут быть разработаны в другие технологические стеки, например: Angular, или React
- статические компоненты можно генерировать из Blazor-кода

Выбор типа компонента может быть определён непосредственно в верстке. Ниже приведены два соответствующих примера:

```csharp
<Message @rendermode="@RenderMode.WebAssembly">
<Message @rendermode="@RenderMode.Server">
```

>Здесь есть тонкий момент: когда мы говорим о Blazor WebAssembly, в режиме AoT приложение представляет собой wasm-код, который действительно выполняется быстро. Однако с отключенный AoT, wasm-Runtime интерпретирует IL-сборки и этот вариант кратно медленнее, чем AoT-вариант. Важно понимать, что Blazor WebAssembly использует JS Interop и изменения в DOM осуществляются через JavaScript-мост. Однако задачи с высокой вычислительной интенсивностью и, вероятно, рендеринг сложного контента может быть кратно быстрее, чем JavaScript-решения (React, Vue.js, и т.д.).
>
>По умолчанию, "чистая" сборка предполагает компиляцию не только приложения, но и Runtime в wasm-код и эта сборка является вычислительно ёмкой. Она выполняется на порядок дольше, чем например транспиляция с TypeScript в JavaScript.
>
>Сам по себе wasm имеет, как сильные, так и слабые стороны. Преимущества:
>
>- полностью изолирован от хоста, т.е. безопасен для браузера
>- не привязан к конкретным языкам программирования, т.е. wasm - это некоторый байт-код. Можно выбрать: Rust, C++, C#, и т.д.
>- платформо-независимый
>- высокая производительность, связанная с низко-уровневым бинарным форматом и глубокой оптимизацией. Wasm использует Ahead-of-Time (AOT) компиляцию, тогда как JavaScript - компилируется "на лету" (JIT-компиляция). У wasm лучше управление памятью, а также он спроектирован для выполнения вычислительно интенсивных операций: обработки изображений, криптографии, научных расчётов, игр. Однако при взаимодействии с DOM, wasm может проигрывать JavaScript из-за Interop
>
>С другой стороны:
>
>- из-за изоляции есть расходы на коммуникации
>- системное окружение ещё не финализировано

Однако существуют риски, связанные с использованием Blazor:

- при публикации приложения осуществляется трансляция IL-сборок в wasm. Это сильно нагружает CI/CD инфраструктуру
- существуют лимиты по объёму памяти, выделяемой браузером виртуальной машине. Ориентировочно, объем heap-а составляет 1 ГБ. Это относительно небольшой объём, который заставляет писать код очень аккуратно и заниматься его оптимизацией. В реальных проектах мы уже получали исключение OutOfMemory, когда разработчик пытался скачивать файл размером 200 МБ в heap-память wasm и затем уже передать его браузеру посредством локального stream-а
...
- Hot Reload в клиентской части [Blazor можно легко поломать](./blazor_hotreload_fix.md)
...
- ошибки в дизайне компонентов в Blazor WebAssembly могут приводить к крайне низко-эффективному рендерингу, корректировать который необходимо вручную
...
- из всех доступных моделей Blazor, значимые бенефиты дают только два: Blazor Server и Blazor WebApp с компиляцией в Wasm. Blazor WebApp без компиляции в wasm работает кратно медленнее React и Vue.js
- [жалобы разработчиков](https://www.reddit.com/r/Blazor/comments/17irng3/blazor_the_end_of_react/?rdt=42081): пользовательский интерфейс часто "замерзает" (freezes), ререндеринг не предсказуемый
...
>
>Обновление март 2025: разработчики FluentUI начали активно отказываться от использования ShadowDOM, возвращаясь к традиционным объектам DOM дерева, кастомизируемым через классы и каскадные таблицы. Вероятно, направление использования теневого DOM-а признано ошибочным.
...
## Что важно для Blazor-приложений

При разработке приложений на Blazor важно понимать жизненный цикл компонента. Мы можем переопределить метод жизненного цикла **ShouldRender**() для управления обновлением UI. Если этот метод возвращает **true**, то компонент перерисовывается.

Важно правильно выбирать размер компонента. Существует эвристическое правило: сначала создаём маршрутизируемый компонент страницы и пишем весь код в нём. В какой-то момент код разрастается, и приходит понимание, что одна часть кода не связана с другой частью. Например, обработка формы может быть никак не связана с отображением таблицы. Это сигнал, чтобы разделить компоненты, извлечь дочерние компоненты и превратить компонент страницы в оркестратор нескольких дочерних компонентов.

Не существует правильного, или неправильного подхода. Требуется опыт, чтобы понять, что работает лучше всего. Одно из полезных правил - выделять то, что связано друг с дургом, и ценить связаность больше, чем размер (количество строк).

С появлением интерактивного режима рендеринга для Blazor-компонентов миы получили гораздо больше гибкости по сравнению с предыдущими версиями Blazor. Мы можем, например, комбинировать интерактивность Blazor Server и Blazor WebAssembly в одном приложении. Для обеспечения гибкости архитектуры необходимо настроить компоненты Blazor так, чтобы они не зависели от режима рендеринга. Т.е. не задавать тип интерактивности внутри каждого компонента, а задавать его только на верхнем уровне. Это позволяет использовать компонент как часть интерактивного приложения Blazor Server и Blazor WebAssembly.

Необходимо изучить правильную обработку событий. При удалении компонента обязательно отписывайтесь от событий.

Что касается связи между компонентами, обратные вызовы событий (EventCallback) являются стандартным способом осуществления  обратного вызова родительного компонента из дочернего компонента.

Blazor предлагает разные варианты управления состоянием. Параметры компонентов - самый простой вариант, за которым следуют каскадные значения и извлечение состояния в специализированные реализации сервисов. Для больших приложений может потребоваться использовать специализированную библиотеку, например, [Fluxor](https://github.com/mrpmorris/Fluxor).

Критичным является использование чёткой, понятной и организованной структуры кода. Необходимо следовать рекомендациям и _conventions_. За основу можно взять новый шаблон web-приложения Blazor в .NET 8.

Необходимо ответственно отнестись к безопасноти и использовать лучшие практики, такие как **OSWASP Top 10**.

Для логирования хода выполнения приложения можно использовать библиотеку Serilog.

Следует всегда стремиться к максимальной простоте кода.
...
- PWA
- Очень высокая производительность вычислений в браузере. Но при этом возросшие затраты на JS Interop в случаях, когда нужно что-то изменять в DOM
...
> Чуть более глубоко объяснить разницу между hostings models можно через подобие. Существует две больших группы программного кода. Одна группа всегда работает в браузере - это интеграционный слой "Blazor.js" и DOM браузера. Вторая группа состоит из: Razor Components, Blazor и Render Tree. Вот эта вторая группа может размещаться как на сервере, так и внутри браузера. В случае Blazor Server - эта группа представляет собой IL-сборки .NET Core; связь этого кода с "Blazor.js" осуществляется посредством SignalR (WebSockets). В случае Blazor WebAssembly - группа представляет собой сборки dotnet.wasm, которые размещаются и работают внутри браузера и обмениваются сообщениями с "Blazor.js" через JS Interop.
>
> В случае использования .NET MAUI всё становится немного сложнее, т.к. за DOM появляются ещё и Native Components, специфические для каждой конкретной операционной системы (macOS, Android, iOS, Tizen).

При использовании модели Blazor Server, в браузере не исполняется никакого wasm-кода - только JavaScript и SignalR, как средство обмена данными между браузером и сервером.
