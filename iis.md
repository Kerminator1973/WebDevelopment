...
Информация об актуальных сеансах может храниться как в базе данных, так и в запущенном процессе. С целью минимизации усилий по запуску web-сервера, можно активировать режим "В процессе" (InProc). Заметим, что управление сессиями осуществляется с использованием системного сервиса ASP.NET State Service.
