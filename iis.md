...
Идея пула приложений состоит в том, что действия сайта по работе с ресурсами осуществляются от имени некоторого пользователя. Указывая запись **AppPool** мы связываем сайт (приложение) с пользователем системы. Этому пользователю мы можем назначить набор прав в системе, например, доступ к папкам файловой системы. Соответственно разные приложения могут действовать от имени некоторого общего пользователя, или каждое приложение может иметь своего собтсвенного пользователя.  Таким образом, "пользователь с именем сайта" в IIS чаще всего — это не реальный пользователь Windows, а идентичность пула приложений.

Кроме этого, в настройках **app pool** можно указать тип окружения (_runtime environment_) необходимого для запуска приложений.
...
![Профиль публикации VS2022](/images/launchProfilesVS2022.png)
...
После перезапуска web-приложения, Visual Studio ищет сайт IIS, который слушает тот же самый порт и предлогает переопределить папку проекта (в управляющей консоли IIS это "Основные настройки" -> "Физический путь"). Рекомендуется согласиться на предложенное изменение. После этого должен заработать запуск и отладка проекта запускаемого через IIS в Visual Studio.
...
3. acme.ru.der.crt – сертификат c публичным ключом сервер для клиента, который может быть использован для проверки серверного сертификата
4. acme.ru.pem - сертификат c публичным ключом сервер для клиента в текстовом виде, пригодном для хранения в ресурсах клиентского приложения
...
Запустив "Диспетчер служб IIS" (inetmgr.exe) следует выбрать соединение (корневой элемент дерева «Подключения»), затем выбрать иконку "Сертификаты сервера" и действие "Импортировать…". Важно указать, что размещать сертификат нужно в  хранилище сертификатов "WebHosting", а не "Personal".
...
### Клиентские приложения и сертификат с публичным ключом сервера

В клиентских приложениях может быть удобнее использовать pem-сертфикаты, которые хранятся в текстовом виде, в кодировке base64.
...
## При использовании доменной аутентификации (Active Directory) сайт отвергает пользователя

Проблема может быть связана с тем, что "по умолчанию", при установке IIS, не устанавливается компонент IIS "Windows Authentication". Для приложений с доменной аутентификацией, настройка "Проверка подлинности" должна быть следующей:

![Настройка сертификатов](/images/iisWindowsAuth.png)

Если в интерфейсе IIS нет опции Windows Authentication, сначала нужно установить компонент. Для этого в Windows следует открыть "Панель управления → Программы и компоненты". Выберите «Включение или отключение компонентов Windows». Затем развернуть:

Internet Information Services → World Wide Web Services → Безопасность.

Отметьте «Windows Authentication» и установить его.

После того, как компонент будет установлен, нужно будет включить "Проверку подлинности Windows" и отключить "Анонимную проверку подлинности".
...
При удалении app pool и сайта, папка с данными пользователя может сохраниться на диске. Для удаления такого пользователя (и вообще ненужных пользователей), следует запустить Powershell в режиме администратора и выполнить команду определения принадлежности папки. Например:

```powershell
$acl = Get-Acl "C:\Users\React-SPCD"
$owner = $acl.Owner
$owner
```

В общем случае, система вернёт текст:

```shell
BUILTIN\Администраторы
```

Далее следует определить состояние учётной записи:

```powershell
Get-CimInstance Win32_UserAccount -Filter "Name = 'React-SPCD'" | Select-Object Name,Domain,Disabled
```

Если вывод пустой — учётной записи нет. В моём случае я получаю пустой вывод, а это значит, что папку можно безболезненно удалить.

Команда удаления папки:

```powershell
Remove-Item -Path "C:\Users\React-SPCD" -Recurse -Force
```

Конфигурации IIS находятся в папке `%windir%\System32\inetsrv\config` в файле "applicationHost.config".
