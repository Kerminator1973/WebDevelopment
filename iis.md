...
Информация об актуальных сеансах может храниться как в базе данных, так и в запущенном процессе. С целью минимизации усилий по запуску web-сервера, можно активировать режим "В процессе" (InProc) - в этом случае все активные сеансы будут потеряны, если web-сервер будет перезапущен.

Отрадно, что в IIS конфигурационный файл "Web.Config" транслируется в поля графического пользовательского интерфейса и их довольно удобно изменять.
