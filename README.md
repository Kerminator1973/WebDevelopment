...
2. Framework [Angular](https://angular.io/) от Google. Бывший лидер. Особенность - все необходимые компоненты уже встроены во Framework
3. [Vue.js](https://vuejs.org/), особенно популярная у китайский товарищей. *Evan You* - один из разработчиков Angular.js
...
Важно знать, что вычисление хэш-кода является односторонней криптографической операцией (*one-way algorithm*), что означает, что можно вычислить хэш-код для пароля, но нельзя вычислить пароль по хэш-коду. Примером алгоритма для вычисления хжш-кода является **SHA512**.
