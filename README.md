...
Другими словами, CORS использует схему обмена сообщениями, в которой участвуют браузер, клиентский код в браузере и сервер. Браузер контролирует правдивость заполнения полей клиентским кодом (javascript), а также чёткое следование протоколу кросс-доменного обмена данными и в случае выявления нарушений, блокирует обмен данными.

На практике чаще всего CORS применяется при использовании Single Page Applications (с использованием React, Vue.js, Angular). В таких решениях SPA часто загружается с использованием одного порта, а backend работает на другой порту. Например, backend, разработанный на .NET Core 5 может обрабатывать запросы на порт 5000, а загрузка SPA может осуществляться через порт 3000, на котором работает Node.js. Поскольку сетевой порт - часть домена, браузер считает somesite.org:3000 и somesite.org:5000 разными доментами и будет блокировать запросы к somesite.org:5000, если их пытается выполнить javascript-код загруженный с somesite.org:3000. Проверка осуществляется посредством дополнительного предзапроса с использованием HTTP-глагола **OPTIONS**. Получая такой запрос сервер сообщает, какие источники javascript-кода являются допустимыми для подобных запросов.
