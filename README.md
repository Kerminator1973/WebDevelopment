...
Основное ограничение при разработке web-сайтов на Node.js - выполнение http-запросов, требующих значительно вычислительно нагрузки. Поскольку Node.js асинхронный, но однопоточный, то если какой-то запрос начнёт выполнять длительные вычисления, другие запросы будут ждать выполнения такого "тяжёлого" запроса. Решается проблема разными способами:

- разбиением всей работы на отдельные блоки, итерация по которым выполняется через setTimeout(), что даёт возможность исполнителю выполнять и другие задачи из event loop
- можно запускать отдельный процесс (процесс уровня операционной системы). Node.js содержит инструменты для взаимодействия между родительским и дочерним процессами (на базе EventEmitter)
