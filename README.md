...
>Однако к интеграции ASP.NET и Identity есть множество вопросов. Не смотря на то, что данные приложения и данные Identity могут сохраняться в одной и той же базе данных, по умолчанию, доступа к таблицам с данными пользователей из LINQ нет. Т.е. включить, например, имя пользователя в запрос данных приложения нельзя. Дополнять данные приложения данными пользователя приходится через API, который так же обращается к СУБД. Таким образом, для подобных запросов требуется выполнить, как минимум на один дополнительный запрос к СУБД больше.  Для того, чтобы обойти ограничение, инженеры Microsoft рекомендуют создавать второй DbContext, в котором явно (фактически, вручную) описываются поля Microsoft Identity в ModelBuilder. Это костыль необходимо сделать очень аккуратно и выполнять вручную синхронизацию модели при переходе на новые версии ASP.NET Core.
