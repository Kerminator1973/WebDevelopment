...
Сервер формирует и передаёт браузеру набор данных, состоящий из:
...
>[Что умеет делать современный браузер](https://github.com/Kerminator1973/WebDevelopment/blob/master/browser_what_it_is.md)
...
>[Что делает браузер при переходе по ссылке](https://itcaat.github.io/after-enter/) - учебные материалы.

>Разделение на front-end и back-end разработчиков не такая уж и условность: в современных web-фреймворках почти повсеместно используется **декларативный подход**, тогда как при разработке back-end применяется **императивный подход**. В декларативном подходе описывается желаемый результат, например, HTML-верстка, какой она должна быть. При этом, отдельные элементы верстки могут не создаваться, или быть невидимыми в зависимости от внешних (по отношению к верстке параметров). Императивный подход описывает путь достижения целей, т.е. команды и операции, которые следует выполнить, чтобы прийти к ожидаемому результату.
>
>Таким образом, работа front-end разработчиков чаще всего сфокусирована на композиции/декомпозиции элементов, тогда как front-end разработчики думают в терминах алгоритмов и инструкций.
...
- Развертывание приложений (Deployment) - DevOps
...
- [Работа с сервером](#fetch) посредством fetch-запросов
...
- Особенности технологического стека [Python](#python)
- Применение [Go](#golang) для web-разработки
...
- набор инструментальных средств: bundler-ы, обфускаторы, транспайлеры, IDE
- серверное ПО, обеспечивающее работу приложения
- используемая СУБД и системы кэширования данных

Классические примеры технологических стеков:
...
Крупные IT-компании часто фиксируют разрешённый набор используемых технологий. Так, например, СБЕР допускает использование Postgres и Java, но не допускает использование .NET Framework (не Core), Microsoft SQL Server и Oracle.
...
>Январь 2026 г.: значительная часть Google Fonts стала платной. Этот фактор нужно принимать во внимание.
...
Набирающая популярность библиотека CSS-стилей - [Bulma](https://bulma.io/). Библиотека поддерживается [DataTables.NET](https://datatables.net/) "из коробки".
...
Вместе с тем, снижение популярности происходит скорее не из-за jQuery как таковой, а из-за перехода от Multi-Page Applications (MPA) к Single-Page Applications (SPA). SPA даёт следующие потенциальные преимущества:
...
>С другой стороны, MPA расходует меньше оперативной памяти, при каждом переходе удаляет функции-подписчики и переменные в замыканиях. Крайне важным свойством MPA в долгоживущих проектах является удобство последовательной миграции на новые версии клиентских библиотек. Предположим, что в 2018 году было написано web-приложение Garmr, в котором использовался Bootstrap 4, актуальный на тот момент версии Datatables.NET и jQuery/UI. В случае MPA, можно переводить на Bootstrap 5/new Datatables.net/jQuery страницу за страницей, т.е. задача миграции может быть разбита на множество небольших, автономных задач.
>
>Ещё одна проблема SPA - загрузка ресурсов при старте приложения может быть чрезмерно интенсивной. Это во многом зависит от культуры разработки команды, применению таких подходов как "ленивая загрузка", bundling, и т.д. Однако есть не следить за десциплиной максимально внимательно, очень легко получить, например, приложение, которое загружает 76 различных ресурсов при старте приложения.
...
- медленный Runtime (если сравнивать с приложениями на C++ и Rust). Использование JIT-компиляции улучшает ситуацию, но эффективность JIT-компиляции и AOT-компиляции несопоставимы
...
>В 2025 году Microsoft завершает работу по [портированию TypeScript на Go](https://habr.com/ru/articles/890056/). Целью проекта является ускорение компиляции (транспиляции) приложений на TypeScript в 10 и более раз. Также портирование позволяит ускорить процесс подготовки проекта к работе с ним в Visual Studio Code. Выбор в пользу Go сделан по следущим причинам: текущий код TypeScript проще всего перевести именно на golang. Благодаря поддержке Go огромного количества платформ (аппаратное обеспечение и операционная система), выбор Go делает TypeScript максимально кросс-платформенным.

Принимая во внимание опыт web-разработчиков, можно говорить о том, что JavaScript вполне можно использовать для небольших проектов. В случае, если проект имеет более 10000 строк кода, использование языков программировния со строгой типизацией становится безальтернативным. Соответственно, основные кандидаты это TypeScript (доминирующий) и C#/Blazor (восходящая звезда).
...
>В 2025 году часто встречаются гибридные приложения, в которых часть компонентов пользовательского интерфейса формируются (rendering) браузером, а часть - сервером.
>
>Мир окончательно ушёл от идеи использования некоторого универсального подхода к разработке - web-приложения могут отличаться друг от друга, как гепард от дельфина. В т.н. intranet-приложениях (enterprise) может использоваться WebSockets с постоянно установленным двухсторонним соединением (полнодуплексное), с преимущественным рендерингом на сервере, что позволяет не реализовывать дополнительный уровень в виде REST API. Гипер-масштабируемые приложения (их относительно мало) могут быть построены с использованием микро-сервисной архитектуры, которая, в действительности, является не технологическим, а организационным шаблоном проектирования. SPA приложения могут быть как online, так и offline. Используя WebAssembly, браузер может выполнять приложения с высокой интенсивностью вычислений. В 2025 году мы находится в точке беспрецендентного технологического разнообразия.

Ещё одна особенность, которая стала крайне важной - использование семантической верстки. Вместо использования тэгов span и div, теперь хорошим тоном является использование: main, header, nav, article, section, footer, aside, и т.д. Пример семантической HTML-разметки:
...
<body>
    <header>
        <h1>Мой сайт</h1>
        <p>Пример семантической HTML-разметки</p>
    </header>
    ...
    <main>
        <section id="home">
            ...
            <p>Это основной контент страницы. Здесь размещается уникальное содержимое, которое не повторяется на других страницах сайта.</p>
        </section>

        <section id="about">
            <h2>О нас</h2>
            <p>Здесь можно рассказать о компании, проекте или авторе сайта.</p>
        </section>
...
Ценность семантической HTML-верстки состоит в том, что такие страницы удобно обрабатывать в системах автоматизации, включая использовании для обучения ИИ.
...
>Январь 2026 г: Svelte и Qwik пока не получили долю рынка сравнимую с React и Vue.js, но их подходы compiled-based и hydration-free уникальные и очень перспективными.
...
**Shadow DOM** - это способ инкапсуляции органов управления, разработчики которых хотели бы, чтобы внешние изменения CSS повлияли бы на внешний вид их компонентов. В частности, Shadow DOM используется при реализации таких органов управления, как выпадающий список (состоит из нескольких элементов), или Range (выбор значения в диапазоне). Если мы попытаемся увидеть Layout через Developer Console, или через селекторы, то мы увидим только внешний контур, например:

```html
<input type="range">
```

Однако, под внешним контуром может быть весьма изобретательная и сложная верстка. При выполнении ряда условий мы можем получить доступ к стилистичесому оформлению этой верстки и, таким образом, поменять отображение системных объектов. Например, мы можем поменять цвет фона у ползунка (range):
...
- 2010-2015 - появились первый SPA: Backbone.js, Ember, Kbockout, Angular.js
...
**CSRF** (Cross‑Site Request Forgery, межсайтовая подделка запроса) — тип атаки, при котором злоумышленник вынуждает авторизованного пользователя выполнить нежелательные действия на веб‑сайте без его ведома. Пример атаки:

- Пользователь авторизуется на сайте банка и получает сессионную cookie
- Злоумышленник создаёт ссылку или страницу с кодом, который отправляет запрос на банковский сайт, который содержит команду перевода денежных средств на некоторый счёт, или команду смены пароля
- Пользователь переходит по ссылке (через фишинговое письмо, сообщение в соцсети и т. д.)
- Браузер автоматически добавляет сессионные cookie к мошенническому запросу — сервер считает его легитимным
- Сервер выполняет действие (перевод денег, смена пароля и т. п.) от имени пользователя.

Типовые решения по защите:

- CSRF‑токены (Synchronizer Token Pattern): для каждой сессии или формы генерируется уникальный случайный токен. Он передаётся в форме (как скрытое поле) и проверяется сервером при получении запроса
- Атрибут SameSite для cookie: браузер не отправляет cookie с запросами с других доменов
- Проверка заголовков Origin и Referer: сервер проверяет, что запрос пришёл с доверенного домена
- Подтверждение критически важных действий: SMS, сообщение по почте, одноразовый пароль, и т.д.
- Double Submit Cookie: токен передаётся и в cookie, и в параметре запроса. Сервер проверяет их совпадение

**XSS** (Cross‑Site Scripting, или межсайтовый скриптинг) — тип атаки на веб‑системы, при которой злоумышленник внедряет в выдаваемую веб‑страницей информацию вредоносный код. Этот код выполняется на компьютере пользователя при открытии страницы и может взаимодействовать с сервером злоумышленника. Чаще всего внедряется JavaScript‑код.

<a name="fetch"></a>

**AbortController** — встроенный в JavaScript класс (появился в ES2018), позволяющий отменять асинхронные операции, включая fetch-запросы.

Основные компоненты:

- new AbortController() — создаёт экземпляр контроллера
- controller.signal — возвращает объект AbortSignal, который передают в fetch
- controller.abort() — запускает отмену операции.

Класс позволяет уведомить сервер о том, что ранее отправленный запрос клиента уже не интересует. 
Важно применять для следующих задач:

- Отмена поисковых запросов при вводе
- Отмена запросов при размонтировании компонента (React)
- Таймаут запроса
- Устранение гонки запросов

**Предотвращение дублирования запросов** — техника, предотвращающая отправку идентичных запросов к серверу в определённый период времени.

Проблемы, которые решает дедубликация:

- избыточная нагрузка на сервер
- перерасход трафика
- "гонка" ответов (приходят в неправильном порядке)
- ухудшение UX из‑за лишних обновлений интерфейса
- потенциальные конфликты данных.

Самое простое решение - cache по URL: сохраняем Promise для каждого уникального URL. Если запрос уже идёт — возвращаем существующий Promise.

**Debounce** для поисковых запросов - отправляет запрос только после паузы в вводе.

**Троттлинг** (throttle) - ограничивает частоту запросов — не чаще одного в указанный интервал.

Комбинация с AbortController - отменяет предыдущие запросы при новом идентичном вызове.

Кеширование с TTL - cохраняет ответы на время, предотвращая повторные запросы.

Существуют продвинутые стратегии предотвращения дедубликации - часто это отдельная большая задача в рамках проектного управления.
...
Даже если оба экземпляра приложения размещаются на одном сервере, разные приложения могут обрабатывать запросы на разных портах машины. Например, http-запросы браузера будут приходить на порт 5000, а Web API будет обрабатывать запросы JavaScript-кода на порту 3000.
...
|         Runtime           |     Язык       |                    Web-сервер                    | IDE                    |
|:-------------------------:|:--------------:|:-------------------------------------------------|:-----------------------|
| ASP.NET Core              |       C#       |       Internet Information Server и Kestler      | Visual Studio, VS Code |
| Node.js, Deno, Bun        | JavaScript, TS |           Express.js, Koa.js, Fastify            | VS Code                |
| Python                    |     Python     |                  Django, Flask                   | PyCharm                |
| Spring, Struts, Hibernate |  Java, Kotlin  | Nginx/Apache с Tomcat, Glassfish, Wildfly, Jetty | IntelliJ, VS Code      |
...
>Многообразие используемых эко-систем связано с их особенностями. Различия состоят в легковесности Runtime (объём), скорости работы, комбинации сложности и эффективности языка программирования, тому, насколько эко-система защищена от ошибок в коде. Так, например, экосистема Go позволяет компилировать код в маленькие по размеру и очень эффективные приложения, которые великолепно подходят для использования в Docker-образах и прекрасно подходят для микросервисной архитектуры. Java, как язык программирования, прощает многие ошибки программистов и очень удобен для middle-ов, но в части развертывания приложений, больше подходит для монолитных приложений, чем для микро-сервисов.
>
>Golang, Rust и, в некоторых случаях, C++ обладают легковесным runtime и позволяют организовать оптимальную, в плане производительности и эффективности работы приложений, облачную инфраструктуру.
>
>Особое влияние в 2025-2026 годах на IT-инфраструктуру оказывал Go, который имеет простой синтаксис и является компилируемым языком. Т.е. одновременно обеспечивается как низкий порог вхождения в эко-систему, так и очень высокая производительность, для некоторых задач соизмеримая с Rust и C++. Благодаря очень низким требованиям с вычислительным ресурсам (ОЗУ, диск, процессор) Go-приложения прекрасно подходят для задач, выполняемых в контейнерах (Docker, Podman), особенно эффективны в микросервисной архитектуре (за исключением задач отладки). В условиях, когда t4g.micro (2 ARM CPU, 2 ГБ ОЗУ) и t3.large (2 CPU x64, 8 ГБ) отличаются по цене на порядок, выбор экосистемы Golang может быть бизнес-критичным.
>
>Эко-система Java зрелая, на ней реализовано огромное количество Enterprise-решений. На рынке труда большое количество специалистов, имеющих значимый опыт разработки для этой экосистемы.
>
>Python - большое количество синтаксического сахара, порог вхождения ниже, чем в эко-системы Java, или .NET Core. Относительно низкая производительность.
...
>На практике, потребность использования именно ORM возникает не всегда. Существует близкий класс решений - **SQL‑query builders**. Такие продукты выполняют генерацию SQL-запросов, предоставляя разработчикам возможность избежать использования синтаксических особенностей конкретной СУБД, а также исключить влияние мелких описок при формировании строк с SQL-запросами. Примером таких решений является [Knex.js](https://knexjs.org/). ORM можно расматривать как инструмент более высокого уровня, который кроме построения запросов умеет выполнять дополнительные действия, такие как: единый источник описания структуры базы данных (Code-First в Entity Framework), а также пакетирование (_batching_) запросов и контроль изменения полей (_mark entities as dirty_).
...
Райян Даль (Ryan Dahl), создатель Node.js осуществляет разработку нового продукта [Deno](https://deno.land/). С сильными и слабыми сторонами обоих *runtimes* можно [ознакомиться здесь](deno.md). Deno разработан, преимущественно, на языке программирования Rust. Deno поддерживает TypeScript "из коробки".

В 2025 появилась поддержка Microsoft Windows для "ускоренного Node.js" - [Bun](https://bun.sh/). Декларируется 2x более высокая производительность работы, но при этом 100% совместимость с Node.js. Bun написан, преимущественно на языке программирования Zig.
...
>Поддержку реляционных СУБД в Entity Framework можно расценивать как близкую к совершенству, но как только вопрос касается СУБД с другими моделями данных (NoSQL, key/value, документо-ориентированная модель по типу ElasticSerach, сетевая модель), поддержка для таких моделей в EF отсутствует. Создаётся впечатление, что для Microsoft важно продвигать только реляционную модель, которая оптимально подходит очень ограниченному количеству приложений. Это выглядит так, что Microsoft "за уши притягивает" Microsoft SQL Server даже в те проекты, в которых использование реляционной модели будет существенно ухудшать потребительские свойства продукта.

Для аутентификации пользователей и обеспечения информационной безопасности, в экосистеме ASP.NET реализована библиотека Microsoft Identity, которая позволяет минимальными усилиями добавить в систему аутентификацию пользователей, а также обеспечить ограничение доступа к страницам с использованием ролевой модели и правил авторизации.

Из других ключевых особенностей C# - поддержка Dependency Injection. Благодаря внедрению зависимостей значительно упрощается работа с базой данных, аутентификацией и авторизацией пользователей.

В качестве web-сервера в Microsoft Windows можно использовать **Microsoft Internet Information Server**. В Runtime реализован встроенный сервер **Kestrel**. Статья по настройке IIS и SQL Server доступна [здесь](iis.md).

Считается, что web-приложения на ASP.NET Core обладают высокой производительностью и надёжностью.

Приложения ASP.NET Core могут работать как под Windows, так и под Linux.

В общем случае, при использовании ASP.NET Core используется C\# для backend и JavaScript/TypeScript для front-end. В качестве альтернативы на front-end можно применить Blazor, что позволит использовать удобную компонентную модель и C\# на стороне клиента. Однако полностью избежать interop с JavaScript не удасться. Более того, существуют разные варианты реализации Blazor: Server-Side, Client-Side и Client-Size Wasm. С точки зрения оформления кода эти варианты выглядят похоже, но реализованы совершенно по разному и имеют разные ограничения.
...
Что касается ASP.NET Identity, то основное ограничение здесь состоит в том, что таблицы ASP.NET Identity находятся в той же самой базе данных, но работать с ними следует не через LINQ, а через отдельный API, что требует смешенных подходов для доступа к данным: API + LINQ. Существует возможность "подкастылить" систему таким образом, чтобы обеспечить доступ к таблицам ASP.NET Identity через LINQ, но это значительно усложняет код и повышает когнитивную нагрузку на разработчика.
...
### Фактор риска для Spring и Java

Экосистема Spring является open-source-проектом — её развивают сообщество и разные компании на разных этапах. Однако ключевые активы и коммерческая поддержка принадлежат коммерческой компании. В 2009 году компанию **SpringSource** приобрела VMware.

Затем, в 2013 году, VMware вместе с материнской компанией EMC и General Electric создали совместное предприятие — Pivotal Software. Именно **Pivotal** стала основной компанией, которая активно развивала экосистему Spring (включая Spring Boot, Spring Cloud и другие проекты) и управляла коммерческой поддержкой.

Позже Pivotal Software вошла в состав Broadcom. Сейчас Broadcom — тот, официально владеет активами и контролирует коммерческую LTS-поддержку Spring.

Основной фактор риска для экосистемы состоит в том, что **ключевые активы принадлежат не профильной компании**. Broadcom уже сильно повлияла на развитие VMware в негативную сторону изменив политику лицензирования и вызвав бегство пользователей в альтернативные решения.

Следует заметить, что Broadcom активно поддерживает антироссийские санкции.
...
### Особенности технологического стека Python

Python занимает определённую нишу серверной web-разработки. Наиболее известные web-сервера: Django и Flask.

Основываясь на информации об особенностях языка, можно утверждать, что это один из самых медленных языков для разработки web-приложений. Однако, если рассматривать web-приложение как совокупность совместно используемых инструментов, включая корпоративные шины, базы данных, и т.д., фактор медленного языка может и не играть решающего значения.

Вместе с тем, у Python низкий порог вхождения, т.е. это предпочтительная эко-система для новичков. Также в Python очень мощная поддержка машинного обучения, что может быть огромным плюсом в проектах, в которых ML обладает большим весом.

>Занимательная информация (пока не нашёл подтверждения в официальных источниках): количество full-time разработчиков языка Python всего 4 человека. Вместе с тем, community огромное и очень сильное. Именно community вносит основной вклад в развитие языка и tooling-а для него.

Tooling - также одно из наиболее быстро растущих направлений в экосистеме Python. Так, например, только среди менеджеров зависимостей есть, как минимум, три альтернативных решения, декларирующих лучшие, чем у **pip** функциональные возможности:

- [uv](https://github.com/astral-sh/uv) - быстрый менеджер пакетов для Python, написанный на языке Rust. Работает в 8-10 раз быстрее, чем pip. UV объединяет возможности нескольких инструментов:
    - установка пакетов и разрешение зависимостей (заменяет pip)
    - управление виртуальными средами (заменяет virtualenv)
...
- [Poetry](https://python-poetry.org/) -это современный инструмент для управления зависимостями и пакетами в Python‑проектах. Он объединяет функции нескольких утилит (pip, virtualenv, setuptools и др.)
- [Hatch](https://hatch.pypa.io/latest/) — современный расширяемый менеджер проектов для Python, созданный для упрощения и стандартизации рабочих процессов разработки. Он объединяет управление виртуальными окружениями, зависимостями, сборкой и публикацией пакетов в едином интерфейсе, следуя стандартам упаковки Python (PEP 517, PEP 518, PEP 621).
...
### Применение GO для web-разработки

В 2025 году в России резко возрасла популярность Golang для разработки web-приложения, включая сервисы в микросервисных архитектурах. Одним из основных популяризаторов Go является Яндекс. Также активно Go продвигают online-школы программирования, в частности [PurpleSchool](https://purpleschool.ru/catalog?directionAlias=backend).
...
Однако, в 2025 году стало появляться мнение о том, что Go не очень подходит для больших проектов. Например, в статье [Почему Big Tech тихонько уходит от Go](https://habr.com/ru/articles/905248/) by Debugged Thoughts, указываются проблемы с использованием языка в больших проектах:

1. Протота не справляется со сложностью. Не совршенные generic-и, отсутствуют продвинутые конструкции функционального программирования, есть проблемы с безопасностью типов и "сопоставлением с образцом"
2. Обработка ошибок на большом масштабе — кошмар
3. Разработчики злоупотребляют моделью конкуреции (goroutines и каналы)
4. Сборщик мусора (GC) оптимизирован, но всё равно вносит непредсказуемые задержки
5. Эко-система не достаточно развита

Одна из областей, в которой позиции Goland очень сильны - "всё, что вокруг контейнерезации": Grafana, Prometeus, и т.д.
...
>PATCH, в отличие от PUT, позволяет изменить не всю сущность (Entity), а только некоторые свойства. Например, у сущности "пользователь" поменять только его роль с "Администратор" на "Аудитор", но не изменять другие поля, такие, как ФИО, подразделение, или дата рождения.
...
>Существует мнение, что шаблон проектирования даёт существенные (зачастую критичные) преимущества только в больших проектах, в которых используется горизонтальное масштабирование, а так же репликация баз данных. Для небольших проектов использование CQRS приводит к усложнению кода и большим затратам на разработку.

Довольно часто совместно с CQRS используют шаблон проектирования **MediatoR** (посредник), который встраивается в контроллер (MVC) и переадрессует запросы специализированным классам, в которых реализованы внутренние классы для выполнения команд и запросов. Основная цель - уменьшить прямые зависимости между компонентами, представляя единую точку связи.

В приведённом ниже примере, это осуществляется посредством вызова Mediator.Send():
...
CQRS с MediatR поедлагает несколько преимуществ:

- согласованная обработка как команд, так и запросов
- поведение конвейнера для регистрации, проверки и обработки ошибок
- чёткое разделение пробле с помощью классов обработчиков
- упрощённое тестирование с помощью изоляции обработичков

Цена удобства - дополнительная абстрации и повышение сложности.
...
1. Защищённость системы: за защиту отвечает специализированные инструменты. Пример: защита от DoS и DDoS атак
2. Сокрытие серверов локальной сети и информации об их характеристиках
3. Reverse Proxy работает с сертификатами безопасности с полноценным использованием PKI (Public Key Infrastructure). Ключевая особенность - подтверждение доверенности сторон (т.е. использование цепочек с использованием Certificate Authority и полномочиями). Другими словами, Reverse Proxy осуществляет шифрование и дешифрование SSL-сообщений
4. Выполняет архивирование ответа сервера (эффективность reverse proxy может быть выше на 15-50%)
5. Осуществляется проверка и подмена полей http-запросов в MIME-заголовке
6. Повышенная производительность и надёжность: Reverse Proxy может распределять запрос между несколькими worker-ами (например, между несколькими копиями Node.js)
7. Осуществляет балансировку нагрузки между несколькими серверами
8. Осуществляет кэширование статического и динамического контента
9. Обеспечивает поддержку современных протоколов, в частности [HTTP/2](http2.md). Поддержка HTTP/2 позволяет использовать одно TCP-соединение для нескольких http-запросов, а так же загружать статические документы с упреждением. Например, CSS-файл может начать загружаться до того, как будет завершена загрузка html-документа
...
Если кратко, то прямой прокси решает следующие задачи:

1. Доступ клиентов локальной сети к Интернет
2. Кэширование и сжатие данных
3. Защита локальной сети от внешнего доступа
4. Обход ограничений
5. Блокировка доступа к определённому контенту
...
Update 2024: в последние года Postman стал активно коммерциализироваться (ни групповая работа, ни автоматизация недоступны бесплатно), что с учётом санкций, существенно снижает привлекательность продукта. В качестве альтернатив рассматриваются: [Bruno](https://www.usebruno.com/), [Thunder Client для VSCode](https://www.thunderclient.com/), [Insomnia](https://insomnia.rest/).

[Charles Proxy](https://www.charlesproxy.com/) - инструмент, который позволяет перехватывать трафик (включая https), подменять сертификаты, body, query, и т.д. Часто используется для отладки web-приложений. Продукт коммерческий, но в течение 30 минут может работать бесплатно.
...
[Nuxt.js](https://nuxt.com/) - популярный фреймворк для Server-Side Rendering для Vue.js.

[Million Lint](https://million.dev/) - Plug-In для Visual Studio Code, который помогает оптимизировать производительность React-приложений.
...
В документе не рассматривались aria-атрибуты, которые в реальной жизни очень важны. В частности, в США и Европе, требование обеспечения возможности работы с сайтом только с клавиатуры (без мыши) является юридическим.
...
Как создать [публично доступный](homesite.md) web-сайт на базе домашней сети, бесплатно. ВНИМАНИЕ! Инструкция не работает с интернет-провайдером **МТС**. В 2026 году не рекомендуется к использованию.
