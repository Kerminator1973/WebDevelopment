...
- удобная методы для выполнения запросов: get, post, ajax, а также вспомогательные методы для сборки параметров формы для отправки из на сервер, посредством AJAX (`form.serialize()`, `form.attr()`)
...
Разработка TYpeScript ведётся в Microsoft и первоначально, этот tool рассматривался как внутренний инструмент для портирования Microsoft Office в облако.
...
Следует заметить, что в некоторые инструментальные средства, например в [Deno](https://docs.deno.com/runtime/manual/advanced/typescript/overview), TypeScript встроен и используется как "_язык первого класса_".
...
При разработке традиционных web-приложений задачи программирования часто разделяют на _frontend_ и _backend_. Для этих частей часто используют разные технологические stack-и. Например, frontend разрабатывается в парадигме Single Page Applications (SPA), с использованием React, Vue.js, Angular, Svelte. Backend, в свою очередь, часто создаётся с использованием технологического стека на базе .NET Core, или J2EE. Различие технологических стеков и структуры задач находит своё отражение и в организационной структуре - команды frontend и backend часто имеют очень слабые организационные связи. Это находит своё отражение и в задачах развертывания приложений для эксплуатации (deployment) - каждая из команд имеет свой собственный CI/CD Pipeline и свой собственный сервер.

Часто, оба приложения размещаются на одном сервере, но hosting-приложений ведётся отдельно для frontend и backend. Например, backend, разработанный на .NET Core может обрабатывать запросы, поступающие на порт 5000, а загрузка SPA может осуществляться через порт 3000, на котором работает Express-сервер (Node.js). Поскольку сетевой порт - часть домена, браузер считает somesite.org:3000 и somesite.org:5000 разными доменами и будет блокировать запросы к somesite.org:5000, если их пытается выполнить javascript-код загруженный с somesite.org:3000.
...
Для того, чтобы backend (второй сервер) мог обработать запросы браузера (часто используются REST API), сервер, выполняющий хостинг приложения с API должен поддерживать CORS, т.е. вспомогательный протокол для предоставления браузеру информацию о разрешениях доступа. Браузер является промежуточным звеном между приложением работающим на рабочем месте клиента (SPA) и сервером. Браузер обязан контролировать действия клиентского кода, который считается легко модифицируемым злоумышленниками. Чтобы браузер разрешил подозрительные действия, сервер должен объяснить ему, что эти подозрительные действия являются легитимными.

### Если потребовалась настройка CORS на уровне приложения ASP.NET Core

На сервере, на который может прийти запрос с другого домена, необходимо настроить разрешения для подобных запросов. Ниже приведён пример настройки для ASP.NET Core:
...
Также можно обратить внимание на тот факт, что разделение SPA и API по разным доменам сложно рационально обосновать, т.к. нагрузка на статические сайты (SPA), обычно несоизмеримо более низкая, чем на динамический код (API). Очень похоже, что наиболее заинтересованными в таком подходе являются облачные сервисы, которым выгодно продать один и тот же сервис два раза.
...
Update 2024: в последние года Postman стал активно коммерциализироваться, что с учётом санкций, существенно снижает привлекательность продукта. В качестве альтернативы рассматривается бесплатная альтернатива - [Bruno](https://www.usebruno.com/).
