...
[Node.js](https://nodejs.org/en/) - это программная платформа, ключевыми компонентами которой является движок **V8** (осуществляющий трансляцию JavaScript-кода) и Runtime (разработанного на C++). JavaScript-код выполняется всегда в одном потоке (основном), но блокирующие операции ввода-вывода Runtime исполняет в отдельных рабочих потоках. Как результат, обеспечивается высокая производительность приложений Node.js, в частности, web-сервера Express.
