...
1. Библиотека [React](https://ru.reactjs.org/) от Facebook. React - это ядро. Для создания полноценных приложений нужны вспомогательные *3d party* компоненты. [Пример](reactappsample.md) простейшего React-приложения. Наиболее популярная система управления состояниями - [Redux](https://redux.js.org/). Redux часто критикуют за многословность (*is very verbose*) и высокий расход вычислительных ресурсов. Альтернатива - [MobX](https://mobx.js.org/README.html)
2. Framework [Angular](https://angular.io/) от Google. Бывший лидер. Проблемы с SEO. Особенность - все необходимые компоненты уже встроены во Framework. Считается "сходящим с дистанции"
3. [Vue.js](https://vuejs.org/), Легковесный. Особенно популярный у китайских товарищей. *Evan You* - один из бывших разработчиков Angular.js
...
Архитектурный подход RESTful описывает принципы построения web-приложений, в которых основную роль играют HTTP Routes, точки входа для обработки http-запросов. Ключевыми особенностями RESTFul являются:

- идентификаторы объектов включаются непосредственно в http-запрос позиционно. Например: /computers/89056, где 89056 является идентификатором объекта, описывающего некоторый компьютер
- результаты работы чаще всего возвращаются в JSON-формате
- HTTP-глаголы используются для того, чтобы отличать запрос изменяющие состояние системы (меняют что-то в базе данных) и не изменяющие её (только получают информацию). Такой подход позволяет эффективно управлять кэшированием данных и легче масштабировать систему, разнося операции разных типов по разным хостам

CRUD можно рассматривать как упрощённый случай использования RESTful. Акроним расшифровывается как Create, Read, Update, Delete/Destroy и ссылается на четыре наиболее универсальных операций работы с, практически, с любой сущностью. Под сущностью подразумевается некоторое понятие из конкретной прикладной области, например: пользователь, счёт в банке, запись в блоге.

Широкое распространение CRUD связано с тем, что он достаточно простой, ему легко научить новичков и в большинстве случаев, web-разработчиком достаточно перечислить список сущностей, чтобы понять ожидаемый результат, объём работы и её сложность.

Тем не менее, многие разработчики разделяют RESTful и CRUD, считая, что CRUD подходит только для очень простых задач, а при возрастании сложности становится анти-паттерном. Эта тема подробно описана в статье [What’s Wrong With Your CRUD APIs— Besides Everything?](https://levelup.gitconnected.com/whats-wrong-with-your-crudy-interfaces-besides-everything-bde4f4c8cb8a), написанной Nicklas Millard.

Применение CRUD в следующих случаях затруднено:

1. Большое количество операций Upload (они вообще не рассматриваются в схеме)
2. Случаи, когда пользователь явным образом указывает уникальный идентификатор ресурса при его создании
3. При использовании групповых операций, которые CRUD вообще не рассматривает
4. Необходимость работы с большим количеством подсущностей (например, есть в социальной сети есть сущность *канал*, в которой есть подсущность *сообщение*). CRUD провоцирует возникновения "Ада подсущностей"
