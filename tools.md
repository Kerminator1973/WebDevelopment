...
В качестве альтернативы можно рассматривать [Apache Solr](https://solr.apache.org/).

Оба продукта (ElasticSearch и Solr) используют библиотеку [Apache Lucene](https://lucene.apache.org/). Эта низкоуровневая библиотека поиска, реализует:

- токенизацию (разбиение текста на слова/токены)
- стемминг (приведение слов к основе)
- лемматизацию (приведение к словарной форме)
- стоп‑слова (фильтрацию общеупотребительных слов)
- n‑граммы и др
