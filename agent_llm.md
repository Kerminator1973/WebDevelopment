...
Российский агент встраиваемый в VSCode - [KodaCode](https://kodacode.ru/). Работает с зарубежными LLM без прокси. Предлагает различные тарифные планы, включая бесплатный доступ. Требует авторизации на GitHub. Бесплатный тариф не предоставляет доступ к внешним LLM. Тариф Pro - 1590/месяц предоставляет возможность доступа китайским LLM (MiniMax M2.5, Qwen3, Kimi K2.5, GLM 4.7 & 5, GPT 5.3 Codex). Можно докупить дополнительные пакеты - за 370 рублей можно купить 250 запросов ко внешним LLM. Есть поддержка IDE JetBrains. Разработчики - бывшие сотрудники СБЕРа, разрабатывавшие GigaCode. Базируется разработка на расширении [Continue](https://www.continue.dev/). После загрузки рекомендуется отключать передачу телеметрии сервису Koda, чтобы избежать избыточной нагрузки сети. Также рекомендуется оключать Autocomplete, который пока не всегда предлагает разумные советы.

Update май 2026: KodaCode в случае использования тарифного плана Про использует инференс Kimi K2.6. Для вайб-кодинга это один из лучших вариантов в РФ.

Kimi Code предлагает несколько тарифных планов, самый дешевый из которых стоит $19/месяц. Moonshot AI (создатель Kimi K2) была основана в 2023 году Яном Чжилинем (Yang Zhilin), бывшим профессором Университета Цинхуа и экс-сотрудником Google и Meta.
...
Точная информация по современным LLM обычно не раскрывается. Предположительно, что Gemini 3 Pro, GPT 5.2, Claude 4.5 кратно мощнее LLM предыдущих поколений. Для сравнения: DeepSeek V3, предположительно, имеет 130 миллиардов параметров.
...
>По данным на ноябрь 2025 года, в флагманской модели GigaChat 3 Ultra Preview 702 миллиарда общих параметров. При этом в генерации каждого токена участвуют только 36 миллиардов активных параметров (около 5% от общего объёма). Модель использует архитектуру Mixture of Experts (MoE), которая позволяет активировать только необходимых «экспертов» для обработки конкретного запроса.

## Каким образом можно получить доступ к LLM

На практике, многие разработчики ПО не оплачивают доступ к LLM, используя бесплатные сервисы. Однако важно понимать коммерческие модели поставщиков услуг доступа к LLM. Для примера, компания Anthropic предоставляет доступ к моделям: Haiku, Sonnet, Opus

- Haiku - быстрая и легковесная. К ней можно получить бесплатный доступ через агрегаторов
- Sonnet - сбалансированная модель. Работает эффективно, существенно дешевле, чем Opus
- Opus - очень мощная, дорогая и медленная

В общем случае, бесплатный доступ к мощным моделям не предоставляется.

Одной из метрик мощности ИИ является количество параметров:

- 3B (3 миллиарда параметров) - слабая, компактная LLM. По сути, демонстрации потенциала технологии, но без реальной пользы
- 7B - обладает практической пользой в решении простых задач
- 15B - практическая польза в решении задач средней сложности
- 35B+ с этого количества параметров начинаются мощные LLM

Следует заметить, что при локальном развертывании системы, на ПК без дискретного ускорителя ИИ, для запуска модели нужно, приблизительно, в два раза больше памяти, чем для системы, в которой вычисления будут осуществлятся на ускорителе.
...
Китайские LLM предоставляют доступ к некоторым своим моделям бесплатно, но есть ограничения по надёжности и скорости работы:

- [DeepSeek](https://www.deepseek.com/) V3.2
- [Kimi K2](https://www.kimi.com/)
- [Qwen](https://chat.qwen.ai/) 3
- [Z.ai](https://chat.z.ai/) разрабатывает GLM 5.2. Есть также [альтернативный сайт](https://www.glm52.net/)

Российские LLM - [GigaChat](https://giga.chat/) и [Алиса](https://alice.yandex.ru/) предоставляют бесплатный доступ. Однако для ряда задач доступ платный. В частности, подписка на Алиса.ПРО стоит 100 рублей/месяц. К сожалению, росийским разработчикам недоступны в полном объёме огромные dataset-ы кода, такие как: GitHub, [LeetCode](https://leetcode.com/), Stack overflow, и т.д. Также доступность оборудования для обучения в РФ на два-три порядка хуже, чем у компаний США, или Китая. Доступность оборудования влияет как на количество параметров LLM, так и на периодичность выпуска новых версий LLM.

По информации Павла Поповича (Техножнец/Paperclip), GigaChat построен на базе DeepSeek и натренерован на уникальном российском dataset-е.

Специалист СБЕРа, отвечающий за исследования в области ИИ Сергей Марков (управляющий директор, начальник управления экспериментальных систем машинного обучения в департаменте общих сервисов "Салют") в интервью "И40: С.С. Марков | Как скоро нас ждёт AGI и возможен ли он вообще?", подтверждает, что:

- СБЕР активно общается с китайскими разработчиками LLM, который в Китае около двадцати
- DeepSeek воспринимается в Китае как народное достояние и компания достаточно закрытая
- СБЕР обучает GigaChat на собственном DataSet-е с нуля. Это стратегический выбор, т.к. есть риск, что веса заимствованных моделей могут перестать нам отдавать в какой-то момент, а обучения "с нуля" требует очень развитых компетенций со специфическими навыками
- Яндекс использует веса модели Qwen и дообучает их на своих данных, что позволяет экономить на выпуске LLM, но делает компанию уязвимой к решениям Qwen в части продолжения предоставления доступа к весам новых моделей
...
>В июле 2026 года Яндекс выступил с утверждением о том, что они разрабатывают свою собственную LLM не используя чужие веса. Утверждения, что они используют веса Qwen не соответствуют действительности.
>
>Следует уточнить, что бесплатная "Алиса" и "Алиса.Про" - это совершенно разные LLM.

### Что если доступ к LLM ограничивает РКН

Существует возможность выполнить запрос к регламентирующим органам (РосКомНадзор) с просьбой снять блокировку на доступ для конкретной SIM-карты. Эта опция доступна бизнесу. В случае, если предоставленные разъяснения являются обоснованными, РКН снимает блокировки для конкретной SIM-карты. Это является распространённой практикой.
...
## В чем проблема генерации кода ИИ
...
- ИИ пишет новую фичу вместо того чтобы внести модификацию в уже разработанную
- ИИ нужно каждый раз заново "объяснять" всю логику проекта

## Анонимизация данных

Для того, чтобы избежать вредоносного использования данных выгруженных в ИИ следует осуществлять обезличивание данных (анонимизацию).
...
Крайне полезной при проведении аналитики может быть построение цепочки рассуждений с использованием LLM. Для активации соответствующего функционала рекомендуется явно указать, что требуется выполнить цепочку рассуждений, потребовав подробно описать каждый шаг. Пример промпта:

```
Тебе нужно решить следующую задачу, используя технику Chain of Thought (цепочка рассуждений). 

Инструкция:
1. Разбей решение на четкие логические шаги
2. Подробно объясни reasoning на каждом этапе
3. Покажи промежуточные вычисления или умозаключения
4. В конце представь финальный ответ

Задача: В магазине продают яблоки. Если 3 яблока стоят 15 рублей, сколько будет стоить 12 яблок?

Твоя задача - решить это, максимально подробно описав каждый шаг своих рассуждений.
```

## Борьба с галлюцинациями

Современные LLM позволяют использовать команду cross-check (перекрёстная проверка), чтобы модель:

- перечислила альтернативные объяснения
- сравнила возможные варианты
- указала уровень уверенности в ответе

Также эффективным является использование LLM web-поиска для актуализации данных.
...
## В чем особенность Thinking LLM

**Обычная LLM**: генерирует ответ напрямую, опираясь на статистические закономерности в языке. Модель подбирает наиболее вероятную последовательность токенов на основе контекста и обучающих данных.

**Thinking LLM**: сначала выстраивает цепочку рассуждений (chain-of-thought, CoT), а затем на её основе формирует итоговый ответ. Процесс имитирует человеческое мышление: анализ → декомпозиция → решение → проверка.

Thinking LLM: проходит специальное обучение на примерах с пошаговыми рассуждениями. Инференс оптимизирован для генерации промежуточных логических шагов. Модель может:

- разбивать сложную задачу на подзадачи
- рассматривать альтернативные подходы
- проводить самопроверку и самокоррекцию

Thinking LLM: способна решать задачи PhD-уровня (математика, программирование, мультишаговые рассуждения), так как:

- планирует действия
- вызывает внешние инструменты (поиск, выполнение кода)
- проверяет гипотезы на основе данных

Пример запроса: "Я разрабатываю систему мониторинга сети банкоматов. Возможно разные варианты отправки команд сервера на банкомат. Например, каждый банкомат может систематически посылать http-запрос на сервер для проверки наличия новых команд, или банкомат может слушать некоторый порт, а сервер мониторинга может посылать команды на такой порт, по мере необходимости. Какой вариант реализации мне следует выбрать?"

Блок рассуждений (Claude Sonnet 4.6 Thinking):

```
The user is asking about architectural choices for a network of ATMs monitoring system, specifically about how to send commands from server to ATMs. They describe two options:

1. Polling: Each ATM periodically sends HTTP requests to the server to check for new commands
2. Push: Each ATM listens on a port, and the monitoring server pushes commands to that port as needed

Let me analyze both approaches in the context of ATM systems and provide a thorough answer.

Key considerations for ATM networks:
- Security (ATMs are high-security devices)
- Reliability
- Network topology (ATMs are typically behind firewalls/NAT)

- Latency requirements
- Scalability

With polling, the ATM initiates outbound requests to check for commands, which is firewall-friendly and doesn't expose ports, but introduces latency tied to the polling interval and creates network overhead. With push, the server can send commands immediately for low latency, though ATMs would need to listen on exposed ports and handle inbound connections—which creates security and firewall challenges.

A middle-ground approach like long polling or WebSockets lets the ATM initiate a persistent connection to the server, avoiding the need to expose ports while still achieving near-real-time command delivery. In real banking systems, security is the primary concern—ATMs shouldn't expose ports to the network—and since they're often behind NAT or firewalls anyway, having them maintain an outbound connection to the server is more practical and secure. I need to finish that thought about recommending a hybrid approach, then walk through why direct server-to-ATM connections fail and what works better instead—long polling for simplicity, WebSockets or MQTT for real systems that need low latency and reliability.
```

Вариант 1 (**банкомат опрашивает сервер**) — подходит, но требует доработки:

Простой polling (например, каждые N секунд) легко реализовать, и он firewall-friendly, так как банкомат сам инициирует исходящее соединение. Однако у него есть недостаток — задержка доставки команды зависит от интервала опроса, а при большом парке банкоматов возникает значительная нагрузка на сервер.

Вариант 2 (**сервер подключается к порту банкомата**) — почему лучше избегать:

Банкоматы, как правило, находятся за NAT и корпоративными firewall-ами, и добраться до них извне крайне сложно. Помимо этого, открытый порт на банкомате — это увеличенная поверхность атаки, что недопустимо с точки зрения безопасности банковских систем. Именно поэтому данный подход применяется крайне редко.

Что реально используется в банковской отрасли:

Лучшим решением являются следующие подходы, часто комбинируемые:

- Long Polling — банкомат отправляет запрос, сервер держит соединение открытым до появления команды (или до таймаута). Команда доставляется практически мгновенно, без лишних запросов.
- WebSockets — банкомат устанавливает постоянное двунаправленное соединение с сервером. Сервер может в любой момент отправить команду без задержки. Соединение инициирует банкомат, что решает проблему NAT и firewall.
- MQTT (Message Queuing Telemetry Transport) — лёгкий протокол pub/sub, изначально разработанный для устройств с ограниченными ресурсами в ненадёжных сетях. Банкомат подписывается на свой топик, сервер публикует команды. Это де-факто стандарт в IoT и встраиваемых системах, отлично подходит для ATM.
- Очереди сообщений (RabbitMQ, ActiveMQ, Kafka) — сервер кладёт команду в очередь конкретного банкомата, банкомат забирает её при подключении. Это даёт гарантию доставки даже при временной потере связи.

Для вашей задачи оптимальным выбором будет MQTT или WebSockets. MQTT предпочтительнее, если банкоматы работают в условиях нестабильной сети — протокол поддерживает QoS-уровни и гарантирует доставку сообщений. WebSockets — если у вас уже есть HTTP-инфраструктура и нужна простота интеграции. В обоих случаях соединение всегда инициирует банкомат, что соответствует требованиям безопасности.
