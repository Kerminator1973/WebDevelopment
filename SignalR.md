...
SignalR предназначен для двухстороннего обмена (полнодуплексный режим) информацией между браузером и web-сервером. В качестве транспорта могут быть использованы технологии: WebSockets, Server-Side events, или http (Long Polling). Передаваемые сообщения могут быть закодированы различными способами, включая **JSON**, или **MessagePackProtocol** (передача информации со сжатием).
