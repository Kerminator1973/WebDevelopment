...
## Streams - концепция потоков данных (НЕ потоков исполнения)
...
Потоки в JavaScript можно разделять на части (_forking streams_), или объединять несколько потоков в один (_merging streams_). Как обычно, на npm есть библиотека, которая упрощает разработку кода, выполняющего forking/merging потоков данных. Эта библиотека называется [multistream](https://www.npmjs.com/package/multistream)
