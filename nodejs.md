...
Следует заметить, что прикладные разработчики обычно не разрабатываают функции, которые выполняют resolve() и reject() - за них это делают разработчики системных библиотек. Однако, может возникнуть задача в которой уже существующий устаревший код, построенный на callback-ах, необходимо (для единообразая используемых подходов) переработать, чтобы поддержать Promises. Такой процесс называется _Promisification_.
