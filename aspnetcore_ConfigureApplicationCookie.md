...
>TODO: проанализировать - можно избежать дополнительного запроса для получения `ExpiredAfter` при загрузке HTML-страницы, если разместить его в невидимом поле в теле документа.

### Почему решение о Logout сложнее, чем кажется

При работе с токенами **AspNetCore.Identity** может осуществлять их обновление, используя _sliding expiration_. Т.е. если на сервер поступает новый http-запрос и текущий токен уже протух больше, чем на половину, Identity обновляет его автоматически. Если клиент обладает информацией о том, сколько времени осталось токену (из заголовка `ExpiredAfter`), а также знает Timeout из настроек сервера, то он может рассчитать когда сработает механизм регенерации токена и послать вспомогательный запрос (ping), единственной ценностью которого является принуждение Identity к регенерации токена.

Например, пользователь нажал кнопку "Отсортировать" и содержимое таблицы автоматически обновилось. Это действие явное проявление активности пользователя и должно сбрасывать тайм-аут бездействия. Именно для подобных ситуаций и нужен ping с учётом ExpiredAfter. Именно эта ситуация не позволяет просто использовать поле "Refresh" в MIME-заголовке.
