...
>Следует заметить, что отметить Refresh из JavaScript-кода в браузере не так просто. По этой причине, если необходимо управлять автоматическим выходом из сессии, лучше реализовать его непосредственно в JavaScript-коде, а в обработчике событие OnValidatePrincipal рекомендуется сформировать дополнительный заголовок, например: "ExpiredAfter".
