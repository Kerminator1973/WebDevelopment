...
**Revocation List**, т.е. список отозванных сессий может храниться в реляционной базе данных, хотя чаще всего для него используют Redis, NoSQL store, или другие базы типа key/value.
...
Однако этот вариант имеет очень значимое ограничение - если операцию перезагрузки страницы потребуется отменить, то сделать это из клиентского JavaScript-кода будет невозможно.

### Альтернативный вариант

Поскольку отменить Refresh из клиентского JavaScript-кода невозможно, лучше выполнять Logout непосредственно в JavaScript-коде. В этом случае, в серверном коде мы должны вместо заголовка "Refresh" добавить наш собственный заголовок, например - "ExpiredAfter". Вот как это можно делается:

```csharp
var secondsToExpire = (int)secondsBetween - 4;
context.HttpContext!.Response.Headers["ExpiredAfter"] = $"{secondsToExpire}";
```

В приведённом выше примере используется асинхронный обработчик события `OnValidatePrincipal`, но в действительности никакой асинхронности в коде нет. Кажется разумным не добавлять избыточные временные затраты. Можно просто переписать код следующим образом:
...
Чтобы выполнять Logout на стороне клиента при протухании токена, нам необходимо получить информацию об оставшемся времени жизни токена от сервера. Сделать это не так просто - мы можем лишь отправить дополнительный запрос на сервер с помощью `fetch`, или `XMLHttpRequest` и уже из заголовка ответа получить необходимую информацию. Например, клиентский код может выглядеть так:

```js
$.ajax({
    url: '/Index?handler=Ping',
    method: 'GET',
...
Объект jqXHR (jQuery XMLHttpRequest), возвращаемый методом `$.ajax()`, является обёрткой над нативным браузерным `XMLHttpRequest`.

Далее, используя value, клиентской код может самостоятельно принимать решение о выполнении Logout.
