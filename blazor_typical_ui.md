...
Метод OnParametersSet() вызывается при изменении значений переменных, которые были переданы как параметры, в родительском компоненте.
...
Внедрение зависимости через интерфейс IDialogService необходимо для того, чтобы мы могли выполнять необходимые действия с модальными диалогами:

```csharp
@inject IDialogService DialogService
```

Директива создаёт экземпляр объекта с именем DialogService, реализующего интерфейс IDialogService, который мы используем в вызове ShowDialogAsync().

Важно отметить, что где-то в иерархии компонентов должен быть определён FluentDialogProvider:

```csharp
<FluentDialogProvider />
```

Он может быть определён в одном из родительских компонентов, либо в текущем компоненте.
