...
В [современных браузерах](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/import) мы можем использовать модульную систему (ES Modules), т.е. импортировать один js-файл из другого. Раньше использовалась система импорта CommonJS (CJS). Визуально понять, какая система используется можно по директиве импорта: **import** - для ES Modules и **require** для CJS.

Ключевое ограничение состоит в том, что этот функционал работает только в том случае, если оба файла относятся к модульной системе, т.е. определены в HTML-коде, как модули. Например:
...
Использование модульной системы (ES Modules) даёт множество преимуществ:

- модули загружаются параллельно, что может ускорить запуск крупных приложений
- позволяет импортировать только нужные части модуля (именованный импорт)
- динамический импорт. Поддерживается через функцию `import()`, которая возвращает промис. Позволяет загружать модули по требованию
- возможность использования tooling-а: Vite, Webpack, и т.д.
