...
Следует заметить, что страница, которая наследует класс **LayoutComponentBase** и использует директиву `@Body` называют **макет Blazor**. Это особенный тип компонента. Также следует заметить, что компоненты макета не содержат директиву `@page`.

Если мы хотим использовать на странице другой макет, то нам следует добавить директиву `@layout`:
...
Макет и описание страницы (#page) объединяются в одну отображаемую страницу.

### Ярлык _Imports.razor

Файл "_Imports.razor" содержит различные директивы Blazor, которые следует применить ко всем компонентам в папке. Чаще всего это директивы `@using`, но можно использовать в том числе директивы `@layout`.
...
Следует заметить, что навигационную панель Bootstrap также можно генерировать динамически используя цикл `@for`, или `@foreach`. Например:
