...
Следует заменить, что добавление конструкций в OnModelCreating() является опциональной. Эта настройка бывает нужна в тех случаях, когда требуется более точная настройка связей, которая не обеспечивается через _conventions_.
